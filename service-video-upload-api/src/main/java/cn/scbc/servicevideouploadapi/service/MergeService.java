...
public class MergeService {
    ...
    public void merge(MergeChunkPoJo mergeChunkPoJo,String storeHouseUUID){
        String storeHousePath = BuildPathUtils.buildPath(local,accessPrefix,storeHouseUUID,"/");
        ...
        if (!chunkFolder.exists()){
            log.error("不存在此分片文件夹或者已上传过-----{}",chunkFolderPath);
    ...
    public void doMerge(Path outPutFile,Integer total,String chunkFolderPath){
        try (FileChannel outputChannel = FileChannel.open(outPutFile,
            ...
            for (int i = 0; i < total; i++) {
                ...
                chunkFileChannel = FileChannel.open(Path.of(chunkPath), EnumSet.of(StandardOpenOption.READ));
