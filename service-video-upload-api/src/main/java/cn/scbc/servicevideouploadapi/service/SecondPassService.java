...
public class SecondPassService {
    ...
    public Boolean isSecondPass(String storeHouseUUID, SecondPassPoJo secondPassPoJo){
        ...
        String chunksPath = BuildPathUtils.buildPath(local,accessPrefix,storeHouseUUID,"/",md5);
        String videoPath = BuildPathUtils.buildPath(local,accessPrefix,storeHouseUUID,"/",md5,secondPassPoJo.getFileType());

        //去数据库看有没有对应md5值的视频
        if (!videoMapper.selectByMd5(storeHouseUUID,md5).isEmpty())
            return Boolean.TRUE;
        ...
        FilesUtils.creatFolder(chunksPath);
